...
def node_name(node: dict[str, Any]) -> str:
    ...
    return str(props.get("name") or f"{node.get('type', 'node')}#{node.get('id')}")
...
def normalize_variable_defs(variable_defs: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for index, variable in enumerate(variable_defs or []):
        ...
        default_value = (
            safe_int(variable.get("defaultValue"), 0, None)
            if data_type == "int"
            else str(variable.get("defaultValue", ""))
        )
        normalized.append({
...
def resolve_variable_value(
    ...
    if variable.get("dataType") == "int":
        return safe_int(value, safe_int(variable.get("defaultValue"), 0, None), None)
...
def _frame_text(frame: dict[str, Any] | None) -> str | None:
    ...
    if frame.get("kind") == "branch":
        ...
        remaining = max(total - frame.get("body_index", 0), 0)
        return f"分支 {node_name(frame['node'])}: {frame.get('branch_label', '未知')}，剩余 {remaining} 个节点"
...
def serialize_state(session: dict[str, Any]) -> dict[str, Any]:
    ...
    variable_values = session.get("vars", {})
...
def _advance_after_subflow(session: dict[str, Any]) -> list[str]:
    ...
    while continuations:
        ...
        if next_index < len(body_ids):
            ...
            if frame.get("kind") == "loop":
                logs.append(
                    f"进入循环：第 {frame.get('iteration', 1)}/{frame.get('loop_count', 1)} 次，第 {frame['body_index']}/{len(body_ids)} 个节点"
                )
            elif frame.get("kind") == "branch":
                logs.append(
                    f"进入分支：{frame.get('branch_label', '未知')}，第 {frame['body_index']}/{len(body_ids)} 个节点"
                )
            return logs

        if frame.get("kind") == "loop" and frame.get("iteration", 1) < frame.get("loop_count", 1):
            frame["iteration"] = frame.get("iteration", 1) + 1
            frame["body_index"] = 0
...
def _write_payload_to_variable(
    session: dict[str, Any],
    target_variable_id: str | None,
    payload: Any,
    logs: list[str],
) -> None:
    variable_values = session.setdefault("vars", {})
    variables_by_id = session.get("variables_by_id", {})
    written, converted = assign_variable_value(target_variable_id, payload, variable_values, variables_by_id)
    if written:
...
def step_once(session: dict[str, Any]) -> tuple[list[str], bool]:
    ...
    callstack = session.setdefault("callstack", [])
    callstack.append(f"{node_type}:{node_name(node)}")
    if len(callstack) > 50:
        del callstack[:-50]
